...
% Initialization
  ...
  while 1
      ...
      if( d > 0 && sigma > 0 )
        if it <= d && d > 1 && it > 1
           s_prod(it:end) = s_prod(it:end)*s_old;
        end      
        ...
        if it > d
            ...
            theta = zetabark*theta ...
                + abs(zetabark*zetabar*s_prod(ix)*s_old) ...
                - zetabark^2;
            
            errveccg(it-d) = sqrt(abs(errveccg(it-d)^2 - 2*theta));
        end
        ...
        ix = mod(it-1, d) + 1;
        if it > d && d > 1
           s_prod = s_prod/s_prod(mod(ix, d) + 1);
           s_prod(ix) = s_prod(mod(ix-2, d)+1)*s_old;
        end
        s_old = s;
      end
